import * as express from "express"
import { db } from "../index"
import { CollectionReference, FieldPath } from "firebase-admin/firestore"
import { Products } from "../types/products"

const collection = "products"

// Get a list of products
export const getProductList = async (
  _request: express.Request,
  response: express.Response
) => {
  try {
    const query: FirebaseFirestore.Query<FirebaseFirestore.DocumentData> =
      db.collection(collection)

    const snapshot: FirebaseFirestore.QuerySnapshot<FirebaseFirestore.DocumentData> =
      await query.get()

    if (snapshot.empty) {
      return response.status(404).send({ data: "No products found" })
    } else {
      const products = snapshot.docs.map((doc) => ({
        _id: doc.id,
        ...doc.data(),
      })) as Products[]
      return response.status(200).send({ data: products })
    }
  } catch (error) {
    return response.status(500).send({ data: "Error retrieving products" })
  }
}

// Endpoint to get a list of products
export const getPaginatedProductList = async (
  request: express.Request,
  response: express.Response
) => {
  try {
    // Extract query parameters from the request
    const { lastId, firstId, toNext } = request.query

    const limit = 10

    // Initialize the query
    let query = db.collection(collection).orderBy(FieldPath.documentId())

    // Apply pagination to the query
    if (toNext == "null") {
      // Initial and filter query
      query = query.limit(
        limit
      ) as CollectionReference<FirebaseFirestore.DocumentData>
    }

    if (toNext === "true" && lastId !== "" && lastId !== "null") {
      // Next button query
      query = query
        .startAfter(lastId)
        .limit(limit) as CollectionReference<FirebaseFirestore.DocumentData>
    } else if (toNext === "false" && firstId !== "" && firstId !== "null") {
      // Previous button query
      query = query
        .endBefore(firstId)
        .limitToLast(
          limit
        ) as CollectionReference<FirebaseFirestore.DocumentData>
    }

    // Execute the query
    const products = await query.get().then((res) =>
      res.docs.map((doc) => ({
        // Extract the product data from the document
        _id: doc.id,
        ...doc.data(),
      }))
    )

    // If no products were found, send a 404 response
    if (products.length === 0) {
      response.status(404).send({ data: "No products found" })
    } else {
      // Send the list of products to the client
      response.status(200).send({ data: products })
    }
  } catch (error: unknown) {
    if (typeof error === "object" && error !== null && "message" in error) {
      response.status(500).send({ data: error?.message })
    }
  }
}

// Get product by id
export const getProductById = async (
  request: express.Request,
  response: express.Response
) => {
  try {
    const id = request.params.id

    const snapshot = await db.collection(collection).doc(id).get()

    if (!snapshot.exists) {
      return response.status(404).send({ data: "No product found" })
    }

    const product = snapshot.data()

    return response.status(200).send({ data: product })
  } catch (error) {
    return response.status(500).send({ data: "Error retrieving product" })
  }
}

// Create product
export const createProduct = async (
  request: express.Request,
  response: express.Response
) => {
  try {
    const body = request.body as Products
    // Add product to the database

    const res = await db.collection(collection).add(body)

    return response.status(201).send({
      data: {
        id: res.id,
      },
    })
  } catch (error) {
    return response.status(500).send({ data: "Product cannot be created" })
  }
}

// Update product
export const updateProduct = async (
  request: express.Request,
  response: express.Response
) => {
  try {
    const id = request.params.id
    const body = request.body

    // Check if product exists
    const snapshot = await db.collection(collection).doc(id).get()

    if (!snapshot.exists) {
      return response.status(404).send({ data: "No product found" })
    }

    // Update product
    await db.collection(collection).doc(id).update(body)

    // Get updated product
    const updatedSnapshot = await db.collection(collection).doc(id).get()
    const updatedItem = updatedSnapshot.data()

    // Send updated product to the client
    return response.status(200).send({ data: { updatedItem } })
  } catch (error) {
    return response.status(500).send({ data: "Product cannot be updated" })
  }
}

// Delete product
export const deleteProduct = async (
  request: express.Request,
  response: express.Response
) => {
  try {
    const id = request.params.id

    // Check if product exists
    const snapshot = await db.collection(collection).doc(id).get()

    if (!snapshot.exists) {
      return response.status(404).send({ data: "No product found" })
    }

    // Delete product
    await db.collection(collection).doc(id).delete()

    // Send response to the client
    return response.status(204).send({ data: "Product was deleted" })
  } catch (error) {
    return response.status(500).send({ data: "Product cannot be deleted" })
  }
}
